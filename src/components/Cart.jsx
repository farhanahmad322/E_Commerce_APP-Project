...
// this is the function to calculate the total price and discount
...
useEffect(() => {
  calculateTotal(); // culate totals whenever the cartItems change
...
return (
  <div className="container mx-auto p-6 flex flex-col lg:flex-row justify-between mt-14">
    <div className="lg:w-2/3">
      <h1 className="text-2xl font-extrabold mb-6 text-center text-white bg-gradient-to-r bg-yellow-700 p-3 rounded-lg">YOUR CART ITEM</h1>
      {removalMessage && (
      ...
      {cartItems.length === 0 ? (
        <div className="flex justify-center items-center h-48">
          <p className="text-xl font-bold text-gray-700 bg-yellow-100 px-4 py-2 rounded-md">
              NO ITEM IS ADDED IN YOUR CART    {/*Whn No procduct is in cart  */}
          </p>
        </div>
      ) : (
        <ul>
          {cartItems.map((item) => (
            <li key={item.id} className="border-b border-gray-300 py-4 flex items-center justify-between">
              ...
              <div className="flex items-center">
                <span className='mx-4 font-bold'>QTY</span>
                <div className="flex items-center border border-gray-300 rounded">

                {/* To increase and decrease the quantity of products */}
                  <button
                    onClick={() => handleQuantityChange(item, (item.minimumOrderQuantity ) - 1)}
                    className="px-2 font-bold p-2"
                  >
                    -
                  </button>
    ...
    <div className="lg:w-1/3 lg:ml-6 mt-8 lg:mt-0">
      <div className="bg-gray-100 p-4 rounded-lg shadow-md">
        ...
        <button className="mt-4 bg-orange-500 text-white py-2 px-4 w-full rounded-lg">PLACE ORDER</button>
      </div>
    </div>
  </div>
);
